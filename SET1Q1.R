...
for(i in 1:nrow(amce_data)){
  if(amce_data$Target.sales[i]>amce_data$Gross.sales[i]){
    print(paste("Target sale is not Achieved for",amce_data$Months[i]))}else if(amce_data$Target.sales[i]==amce_data$Gross.sales[i]){
      print(paste("Target sale is Achieved exactly as gross sale for",amce_data$Months[i]))
    }else{
      print(paste("Target sale is  achieved for",amce_data$Months[i]))
    }
  }
